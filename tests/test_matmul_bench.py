...
from matmul_bench import environment, gflops, run  # noqa: E402


def test_gflops_formula():
    # A 1000 x 1000 multiply is 2e9 flops, so one second is 2 GFLOP/s.
...
def test_run_is_a_correct_multiply():
    ...
    _, checksum = run(128, 1, np.dtype("float64"), seed=0)
    assert checksum == pytest.approx(float((a @ b)[0, 0]), rel=1e-12)


def test_environment_records_hostname():
